...
function modeShapeValue(modeIndex, depth, waterDepth, model) {
  ...
  const imaginary = model === "krakenc"
    ? 0.075 * Math.sin((order + 0.35) * Math.PI * normalizedDepth) * envelope
    : 0;
...
function makeModes(params, environment) {
  ...
  for (let mode = 0; mode < modeCount; mode += 1) {
    ...
    const imaginary = -(params.model === "krakenc" ? 1.0 : 0.24) * 1.2e-7 * (1 + 20 * (mode / Math.max(1, modeCount - 1)) ** 3);
...
function loadNormalPackage() {
  normalPackagePromise ??= import("@openocean/field-normal-mode-kraken");
...
async function normalSolver() {
  if (!normalSolverPromise) {
    const pending = loadNormalPackage().then(({ Kraken }) => (
      Kraken.create(Kraken.recommendedRuntime())
    ));
...
async function makeNativeInput(params, modeLimit) {
  if (params.model !== "kraken") {
    throw new Error("当前 WASM 版本只启用 Kraken；Krakenc 将在后续包中接入");
  }
  const {
...
export async function parseKrakenEnvironment(input) {
  ...
  if (typeof solver.parseEnvironment !== "function") {
    throw new Error("Kraken 2.0 package does not include the legacy ENV/FLP text parser");
  }
  return solver.parseEnvironment(input);
}
