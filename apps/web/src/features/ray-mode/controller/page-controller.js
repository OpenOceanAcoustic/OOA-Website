...
import { parseEnvironmentFiles } from './environment-import.js';
import {
...
function params() {
  ...
  if(result.profile==='custom'||result.profile==='env'){
    const environment=result.profile==='env'?state.importedEnvironment:state.customEnvironment;
    result.ssp_points=(result.profile==='env'?state.importedEnvironment?.sspPoints:state.customSSP)?.map(point=>[point[0],point[1]])??[];
    if(environment){
