...
function bathymetryDepth(rangeFraction, maximumDepth, model) {
  if (model === "ram") return maximumDepth * (0.91 + 0.015 * Math.sin(rangeFraction * Math.PI * 2));
  const seamount = Math.exp(-Math.pow((rangeFraction - 0.58) / 0.16, 2));
  const slope = model === "rams" ? 0.045 * rangeFraction : 0.02 * rangeFraction;
  return maximumDepth * (0.93 - 0.28 * seamount - slope);
}
...
function baseTransmissionLoss(rangeM, depthM, bathymetryM, params) {
  ...
  const elastic = params.model === "rams" ? 2.2 * Math.sin(0.018 * depthM - 0.00019 * rangeM) : 0;
  return clamp(spreading + channel + direct + interference + bottomPenalty + elastic, 58, 124);
}
...
function truncationPattern(rangeFraction, depthFraction, bathymetryFraction, params) {
  ...
  const modelScale = params.model === "rams" ? 1.25 : params.model === "ramgeo" ? 1.12 : 1;
  return modelScale * (0.42 * angleStress + 0.78 * accumulated + 0.35 * boundaryStress);
}
...
async function demonstrationResult(input, reason) {
  ...
  const params = {
    model: ["ram", "ramgeo", "rams"].includes(input.model) ? input.model : "ram",
...
function loadNativePackage() {
  nativePackagePromise ??= import("@openocean/field-pe-ram");
  return nativePackagePromise;
}

async function nativeSolver() {
  nativeSolverPromise ??= loadNativePackage().then(({ RAM }) => RAM.create());
...
async function calculateNativeSweep(params) {
  if (params.model !== "ram") {
    throw new Error("当前 WASM 版本只启用 RAM；RAMGeo/RAMS 将在后续包中接入");
  }
  const maximumRangeM = params.maximumRangeKm * 1000;
...
export async function parseRamEnvironment(input) {
  ...
  const solver = await nativeSolver();
  return solver.parseRamIn(input.text);
}
