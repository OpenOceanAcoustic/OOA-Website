...
const state = { data: null, animation: 0, raf: null, request: 0, lossImage: null, eigen: null, eigenRequest: 0, customSSP: initialSSP, customInitialized: false, sspDrag: -1, receiverDragging: false, receiverPreview: null };
const controls = {
...
const canvases = { ssp: $('sspCanvas'), ray: $('rayCanvas'), loss: $('lossCanvas'), eigen: $('eigenCanvas'), arrival: $('arrivalCanvas') };
...
function drawArrivals() {
  const {ctx,w,h}=fitCanvas(canvases.arrival);ctx.clearRect(0,0,w,h);ctx.fillStyle='#06161f';ctx.fillRect(0,0,w,h);const rays=state.eigen?.eigenrays||[],a={l:43,r:14,t:18,b:30};a.pw=w-a.l-a.r;a.ph=h-a.t-a.b;
  ...
  if(!rays.length){ctx.textAlign='center';ctx.fillText('NO EIGENRAYS FOUND',a.l+a.pw/2,a.t+a.ph/2);return;}
  const times=rays.map(r=>r.travel_time_s),min=Math.min(...times),max=Math.max(...times),span=Math.max(.08,max-min),lo=min-span*.08,hi=max+span*.08,maxAmp=Math.max(...rays.map(r=>r.amplitude));
  ...
  ctx.fillText('到达时间 / s',a.l+a.pw/2,h-3);ctx.save();ctx.translate(10,a.t+a.ph/2);ctx.rotate(-Math.PI/2);ctx.fillText('归一化幅度',0,0);ctx.restore();
}

function renderEigenSummary() {
  const data=state.eigen;if(!data)return;const rays=data.eigenrays,maxResidual=rays.length?Math.max(...rays.map(r=>Math.abs(r.residual_m))):0;
  $('coarseMiss').textContent=data.nearest_coarse_miss_m.toFixed(1)+' m';$('exactResidual').textContent=maxResidual.toFixed(3)+' m';$('eigenCount').textContent=rays.length+' paths';$('eigenIterations').textContent=data.iterations+' iter';$('coherentTl').textContent=data.coherent_tl_db.toFixed(2)+' dB';$('incoherentTl').textContent=data.incoherent_tl_db.toFixed(2)+' dB';
  $('arrivalRows').innerHTML=rays.length?rays.map(ray=>`<tr><td>E${String(ray.id).padStart(2,'0')}</td><td style="color:${eigenColor(ray)}">${ray.kind}</td><td>${ray.launch_angle.toFixed(4)}°</td><td>${ray.travel_time_s.toFixed(4)} s</td><td>${ray.phase_deg.toFixed(1)}°</td><td>${Math.abs(ray.residual_m).toFixed(3)} m</td></tr>`).join(''):'<tr><td colspan="6">当前角度范围内未发现本征声线，请调整接收点。</td></tr>';
}
...
async function runEigen() {
  ...
  const names={munk:'Munk 深海声道',surface:'表层跃变',constant:'等声速水体',custom:'自定义 500 m 节点'};$('eigenEnv').textContent=`${names[p.profile]} · 声源 ${p.source_depth.toLocaleString('zh-CN')} m · ${p.frequency} Hz`;$('eigenStatus').classList.add('eigen-running');$('eigenStatus').querySelector('span').textContent='正在搜索根区间';$('eigenRun').disabled=true;
  try{const res=await fetch('/api/eigenrays',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(p)});if(!res.ok)throw new Error(`HTTP ${res.status}`);const data=await res.json();if(token!==state.eigenRequest)return;state.eigen=data;state.receiverPreview=null;state.receiverDragging=false;canvases.eigen.classList.remove('dragging');drawEigen();drawArrivals();renderEigenSummary();$('eigenStatus').querySelector('span').textContent=`精确求解完成 · ${data.compute_ms.toFixed(1)} ms`;$('eigenStatus').classList.remove('eigen-running');}
...
async function run() {
  ...
  try {
    ...
    if(!res.ok)throw new Error(`HTTP ${res.status}`);const data=await res.json();if(token!==state.request)return;state.data=data;$('bottomReflectionLoss').textContent=data.bottom.reflection_loss_db.toFixed(2)+' dB';buildLossImage();drawSSP();animate();
...
$('runButton').addEventListener('click',run);$('replayButton').addEventListener('click',animate);
...
syncLabels();drawSSP();drawRay();drawLoss();drawEigen();drawArrivals();run();runEigen();
