import type { NormalModePageResult, NormalModeSingleField } from "./public-types";
...
type Modes = NormalModePageResult["modes"];
...
function complexMultiply(leftReal: number, leftImaginary: number, rightReal: number, rightImaginary: number): Complex {
  return [
...
function complexDivide(leftReal: number, leftImaginary: number, rightReal: number, rightImaginary: number): Complex {
...
function complexSquareRoot(real: number, imaginary: number): Complex {
...
function modeShapeSample(modes: Modes, modeIndex: number, depthIndex: number): Complex {
...
function interpolateModeShape(modes: Modes, modeIndex: number, depthM: number): Complex {
...
function validateResult(result: NormalModePageResult, modeIndex: number): void {
  const { modes, field } = result;
 ...
 * TL_m = -20 log10 |p_m|
 */
export function synthesizeSingleModeField(result: NormalModePageResult, modeIndex: number): NormalModeSingleField {
